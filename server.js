...
app.get('/api/devices', async (req, res) => {
  try {
    ...
    const now = Date.now();
    const TIMEOUT_MS = 25 * 1000; // 25s heartbeat threshold
    const MAX_OFFLINE_RETAIN_MS = 10 * 60 * 1000; // 10 minutes max offline retention
    const deviceList = [];
      ...
      if (roomData) {
        if (roomData.devices && typeof roomData.devices === 'object') {
          for (const [devId, devObj] of Object.entries(roomData.devices)) {
            if (devId === 'DEV-LOCAL' || devId === 'ADMIN') continue;
            if (devObj && typeof devObj === 'object') {
              const lastSeen = devObj.timestamp || devObj.lastSeen || 0;
              const isStillOnline = Boolean(devObj.isOnline) && (now - lastSeen < TIMEOUT_MS);

              // Don't clutter devices list with devices gone for more than 10 mins
              if (lastSeen > 0 && (now - lastSeen > MAX_OFFLINE_RETAIN_MS)) {
                continue;
              }
              ...
              deviceList.push({
                ...
                isCameraActive: isStillOnline ? Boolean(devObj.isCameraActive) : false,
                isAudioActive: isStillOnline ? Boolean(devObj.isAudioActive) : false,
                isScreenActive: isStillOnline ? Boolean(devObj.isScreenActive) : false,
                lastSeen,
                ipAddress: devObj.ipAddress || '127.0.0.1'
              });
            }
          }
        } else if (roomData.status && roomData.status.id !== 'DEV-LOCAL' && roomData.status.id !== 'ADMIN' && roomData.status.deviceId !== 'DEV-LOCAL') {
          const lastSeen = roomData.status.timestamp || roomData.status.lastSeen || 0;
          const isStillOnline = Boolean(roomData.status.isOnline) && (now - lastSeen < TIMEOUT_MS);
          ...
          if (lastSeen > 0 && (now - lastSeen <= MAX_OFFLINE_RETAIN_MS)) {
            deviceList.push({
              ...
              isOnline: isStillOnline,
              isCameraActive: isStillOnline ? Boolean(roomData.status.isCameraActive) : false,
              isAudioActive: isStillOnline ? Boolean(roomData.status.isAudioActive) : false,
              isScreenActive: isStillOnline ? Boolean(roomData.status.isScreenActive) : false,
              lastSeen,
